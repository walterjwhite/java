...
public class JavaMailContactUtil {
    public static Set<EmailEmailAccount> addEmailContacts(final Message message) throws MessagingException {
        final Set<EmailEmailAccount> emailEmailAccounts = new HashSet<>();

        createEmailContacts(emailEmailAccounts, message, Message.RecipientType.TO, EmailRecipientType.To);
        createEmailContacts(emailEmailAccounts, message, Message.RecipientType.CC, EmailRecipientType.Cc);
        createEmailContacts(emailEmailAccounts, message, Message.RecipientType.BCC, EmailRecipientType.Bcc);
    ...
    public static void createEmailContacts(
            ...
            final EmailRecipientType emailRecipientType) throws MessagingException {

        if (message.getRecipients(recipientType) != null) {
            for (final Address address : message.getRecipients(recipientType)) {
                try {
                      emailEmailAccounts
                      .add(new EmailEmailAccount(getEmailAccount(address), null,
                     emailRecipientType));
