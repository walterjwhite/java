...
public class CitrixServiceImpl implements CitrixService {
    @Override
    public void run(final CitrixSession citrixSession, final AbstractCitrixExtension citrixExtension) throws Exception {
        try (final WebClient webClient = new WebClient()) {
            ...
            if(citrixSession.getContextValue(HtmlPage.class) != null) {
                LOGGER.warn("current page source: \n{}", citrixSession.getContextValue(HtmlPage.class).asXml());
            }
        }
    }

    public boolean authenticate(final WebClient webClient, final CitrixSession citrixSession) throws IOException {
        webClient.getOptions().setJavaScriptEnabled(true);
        webClient.getOptions().setCssEnabled(false);

        webClient.waitForBackgroundJavaScript(citrixSession.getBackgroundJavascriptTimeout());

        final HtmlPage page = webClient.getPage(citrixSession.getUrl() + "/logon/LogonPoint/tmindex.html");
        ...
        final HtmlForm form = Util.waitForValue(() -> page.getForms().getFirst(), citrixSession.getPageLoadTimeout());
        if (form == null) {
        ...
        tokenInput.type(citrixSession.getCitrixCredentials().getToken() + citrixSession.getCitrixCredentials().getEphemeralToken());
    ...
    protected boolean isAuthenticated(final CitrixSession citrixSession) {
        final HtmlElement userMenuButton = (HtmlElement)Util.waitForValue(() -> citrixSession.getContextValue(HtmlPage.class).getElementById("userMenuBtn"), citrixSession.getLoginTimeout());
        return userMenuButton != null;
    }

    protected void useLightVersion(final CitrixSession citrixSession) throws IOException {
        final DomElement useLightVersion = Util.waitForValue(() -> citrixSession.getContextValue(HtmlPage.class).getElementById("changeclient-use-light-version"), 10_000);
    ...
    protected void fetchWeb(final WebClient webClient, final CitrixSession citrixSession) throws IOException {
    ...
    public HtmlPage launch(final CitrixSession citrixSession, final int index) throws IOException {
        ...
        final List<Object> matchingElements = Util.waitForValue(() -> currentPage.getByXPath(String.format("//*[@id=\"home-screen\"]/div[2]/section[5]/div[5]/div/ul/li[%d]/a[1]/img", index)), 15_000);
        if(matchingElements == null) {
            throw new IllegalStateException("no elements matched");
        }
        if(matchingElements.size() != 1) {
            throw new IllegalStateException("matched an unexpected number of elements: " + matchingElements.size());
        }
