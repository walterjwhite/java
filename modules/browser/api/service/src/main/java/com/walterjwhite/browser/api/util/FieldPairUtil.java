...
public interface FieldPairUtil {
  ...
  static FieldSecret get(final FieldPair fieldPair, final WebCredentials webCredentials) {
    return Arrays.stream(webCredentials.getFieldSecrets())
        .filter(fieldSecret -> fieldSecret.getSecret().getKey().equals(fieldPair.getId()))
        .findFirst()
        .orElseThrow();
  }
}
