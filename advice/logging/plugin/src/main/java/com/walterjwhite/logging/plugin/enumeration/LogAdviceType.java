  ...
  General {
    ...
    public Class doGetAdvice(final MethodDescription methodDescription) {
      ...
      if (generalTypeOptional.isPresent()) {
        final boolean isResultSensitive = methodDescription.getDeclaredAnnotations().ofType(Sensitive.class) != null;
        if(isResultSensitive) {
          return generalTypeOptional.get().getSensitiveAdviceClass();
        }
