...
public class CloseablePlugin extends Plugin.ForElementMatcher implements Plugin.Factory {
  public CloseablePlugin() {
    super(ElementMatchers.not(ElementMatchers.isAbstract()).and(ElementMatchers.isSubTypeOf(AutoCloseable.class)));
  }
  ...
  public DynamicType.Builder<?> apply(
    ...
    for (final MethodDescription.InDefinedShape methodDescription :
        typeDescription
            .getDeclaredMethods()
            .filter(includeCloseableEligibleMethods())) {
      LOGGER.warn("automatically closing: " + typeDescription.getName());
      builder =
