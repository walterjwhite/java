...
public class DelayAgentHandler implements AgentHandler {
  ...
  public void initialize(final AgentApplicationInstance agentApplicationInstance) {
    ...
    agentApplicationInstance
        .getReflections()
        .getMethodsAnnotatedWith(Delayed.class)
        .forEach(delayedMethod -> setupDelay(agentBuilder, delayedMethod));
  }

  public static void setupDelay(final AgentBuilder agentBuilder, final Method delayedMethod) {
    LOGGER.info(
        "Installing DelayAdvice on {}.{}",
        delayedMethod.getDeclaringClass(),
        delayedMethod.getName());

    agentBuilder
        .type(ElementMatchers.is(delayedMethod.getDeclaringClass()))
        .transform(
