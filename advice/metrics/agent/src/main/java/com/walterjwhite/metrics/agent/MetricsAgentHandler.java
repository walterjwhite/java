...
public class MetricsAgentHandler implements AgentHandler {
  ...
  public void initialize(final AgentApplicationInstance agentApplicationInstance) {
    ...
    agentApplicationInstance.getReflections().getMethodsAnnotatedWith(Metrics.class).forEach(metricsMethod -> setupResilience4j(agentBuilder, metricsMethod));
  }

  public static void setupResilience4j(
          final AgentBuilder agentBuilder, final Method metricsMethod) {
    Arrays.stream(metricsMethod.getAnnotation(Metrics.class).value()).forEach(metricsType -> setupMetrics(agentBuilder, metricsMethod, metricsType));

  }

  public static void setupMetrics(final AgentBuilder agentBuilder, final Method metricsMethod, final MetricsType metricsType) {
    final Class interceptorClass = null;
    LOGGER.info("Installing {} on {}.{}", interceptorClass, metricsMethod.getDeclaringClass(), metricsMethod.getName());

    agentBuilder.type(ElementMatchers.is(metricsMethod.getDeclaringClass()))
            .transform((builder, type, classLoader, module, other) ->
                    builder.method(ElementMatchers.named(metricsMethod.getName())).intercept(MethodDelegation.to(interceptorClass)));
  }
}
