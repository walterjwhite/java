...
public class TimeoutAgentHandler implements AgentHandler {
  ...
  public void initialize(final AgentApplicationInstance agentApplicationInstance) {
    ...
    agentApplicationInstance.getReflections().getMethodsAnnotatedWith(TimeConstrained.class).forEach(timeConstrainedMethod -> setupTimeConstrained(agentBuilder, timeConstrainedMethod));
  }

  public static void setupTimeConstrained(
          final AgentBuilder agentBuilder, final Method timeConstrainedMethod) {
    LOGGER.info("Installing TimeConstrained on {}.{}", timeConstrainedMethod.getDeclaringClass(), timeConstrainedMethod.getName());

    agentBuilder.type(ElementMatchers.is(timeConstrainedMethod.getDeclaringClass()))
    .transform((builder, type, classLoader, module, other) ->
            builder.visit(Advice.to(TimeoutAdvice.class).on(ElementMatchers.is(timeConstrainedMethod))));
  }
}
