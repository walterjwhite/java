...
public class TimeoutPlugin extends Plugin.ForElementMatcher implements Plugin.Factory {
    ...
    super(
        ElementMatchers.declaresMethod(
            ElementMatchers.isAnnotatedWith(TimeConstrained.class)
                .and(ElementMatchers.not(ElementMatchers.isConstructor()))).and(ElementMatchers.isSubTypeOf(TimeConstrainedMethodInvocation.class)));
  }
  ...
  public DynamicType.Builder<?> apply(
    ...
    for (MethodDescription.InDefinedShape methodDescription :
        typeDescription
            .getDeclaredMethods()
            .filter(includeTimeoutEligibleMethods())) {
  ...
  private static Junction<MethodDescription> includeTimeoutEligibleMethods() {
    return ElementMatchers.not(ElementMatchers.isBridge()
            ...
            .and(ElementMatchers.isConstructor())).and(ElementMatchers.isAnnotatedWith(TimeConstrained.class));
  }
