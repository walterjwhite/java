...
import com.walterjwhite.heartbeat.HeartbeatAdvice;
...
public class HeartbeatAgentHandler implements AgentHandler {
  ...
  public void initialize(final AgentApplicationInstance agentApplicationInstance) {
    ...
    agentApplicationInstance.getReflections().getMethodsAnnotatedWith(Heartbeat.class).forEach(heartbeatMethod -> setupHeartbeat(agentBuilder, heartbeatMethod));
  }

  public static void setupHeartbeat(
          final AgentBuilder agentBuilder, final Method heartbeatMethod) {
    LOGGER.info("Installing HeartbeatAdvice on {}.{}", heartbeatMethod.getDeclaringClass(), heartbeatMethod.getName());

     agentBuilder.type(ElementMatchers.is(heartbeatMethod.getDeclaringClass()))
             .transform((builder, type, classLoader, module, other) ->
                     builder.visit(Advice.to(HeartbeatAdvice.class).on(ElementMatchers.is(heartbeatMethod))));
  }
}
