...
public class HeartbeatPlugin extends Plugin.ForElementMatcher implements Plugin.Factory {
  public HeartbeatPlugin() {
    super(ElementMatchers.declaresMethod(ElementMatchers.isAnnotatedWith(Heartbeat.class)).and(ElementMatchers.isSubTypeOf(Heartbeatable.class)));
  }
  ...
  public DynamicType.Builder<?> apply(
    ...
    for (MethodDescription.InDefinedShape methodDescription :
        typeDescription
            .getDeclaredMethods()
            .filter(includeHeartbeatEligibleMethods())) {
      LOGGER.warn("implementing heartbeat on {}", methodDescription.getName());
      builder =
