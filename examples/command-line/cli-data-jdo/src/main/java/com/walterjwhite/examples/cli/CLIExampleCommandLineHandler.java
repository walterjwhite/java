...
public class CLIExampleCommandLineHandler implements CommandLineHandler {
  ...
  public void run(final String... arguments) throws SystemException {
    ...
    ((PersistenceManager)repository.getDelegate()).addInstanceLifecycleListener(
        new SampleLifecycleListener(repository.getDelegate()));
  ...
  protected void saveEmail() {
    ...
    final Email email = Email.builder()
            ...
            .body("body goes here")
            .from(new PrivateEmailAccount(new EmailAccount().withName("Name").withEmailAddress("email@email.com")))
            .messageId("1234")
  ...
  protected void find(final Set<Integer> ids) {
    JDOQLTypedQuery<Entity> greetingQuery = ((PersistenceManager)repository.getDelegate()).newJDOQLTypedQuery(Entity.class);
