...
public class PathRecursion {
  ...
  public boolean pathExistsBetween() {
    if (isOutsideBoundary(start)) {
      return false;
    }

    if (isOutsideBoundary(end)) {
      return false;
    }
  ...
  protected int[] move(final int[] coordinates, final Direction direction) {
    if (isOutsideBoundary(coordinates)) {
      throw new IllegalArgumentException("outside boundary");
    }

    if (hitRock(coordinates)) {
      throw new IllegalArgumentException("hit rock");
    }

    if (traversed.add(new Vector(coordinates[0], coordinates[1], direction))) {
      return coordinates;
    }
