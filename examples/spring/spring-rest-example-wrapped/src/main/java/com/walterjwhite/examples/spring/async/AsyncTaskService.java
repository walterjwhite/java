...
import com.walterjwhite.examples.spring.async.model.TaskStatus;
...
public class AsyncTaskService implements DisposableBean {
    ...
    this.executor =
        Executors.newFixedThreadPool(
            ...
            r -> {
              ...
              t.setDaemon(true);
              t.setName("async-worker-" + workerCounter.getAndIncrement());
              return t;
            });
  ...
  private void runTask(TaskInfo task) {
    try {
      ...
      for (int i = 0; i < total; i++) {
        if (Thread.currentThread().isInterrupted() || task.getStatus() == TaskStatus.CANCELLED) {
  ...
  public String submit(String name, int durationSeconds) {
    String id = NanoIdUtils.randomNanoId(8);
    TaskInfo task =
        new TaskInfo(id, name == null ? "task-" + id.substring(0, 6) : name, durationSeconds);
    allTasks.put(id, task);
    boolean added = queue.add(task);
    log.info(
        "Submitted task id={} name={} duration={} addedToQueue={}",
        id,
        name,
        durationSeconds,
        added);
