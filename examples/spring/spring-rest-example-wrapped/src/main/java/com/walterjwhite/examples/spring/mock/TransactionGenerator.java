...
public class TransactionGenerator {
  ...
  @Value("${mock.transactions.count:2000}")
  ...
  public List<Transaction> generate() {
    ...
    for (int i = 0; i < random.nextInt(transactionCount); i++) {
      ...
      transactions.add(
          new Transaction(
              ...
              getCusip(random),
              transactionDate,
              getTransactionType(random),
              getTransactionStatus(random),
              getQuantity(random),
              getCommissionFee(random),
              getPricePerShare(random),
              getBrokerId(random, 100),
              getClientId(random, 10000),
            ...
            random.nextInt(
                (int)
                    (LocalDate.of(2025, 12, 31).toEpochDay()
                        - LocalDate.of(2008, 1, 1).toEpochDay())));
  }
