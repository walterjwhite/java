...
public class TransformerAgentHandler implements AgentHandler {
  ...
  public void initialize(AgentApplicationInstance agentApplicationInstance) {
    ...
    try {
      for (final Class transformerClass :
          getTransformerClasses(agentApplicationInstance.getReflections())) {
        initTransformer(
            agentBuilder,
            agentApplicationInstance.getSerializationService(),
            agentApplicationInstance.getInstrumentation(),
            transformerClass);
      }
    } catch (Throwable t) {
